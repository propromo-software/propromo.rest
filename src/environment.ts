...
export const PAT_SALT = process?.env?.PAT_SALT!;
export const OPEN_SOURCE_PROGRAM_PATS = process?.env?.OPEN_SOURCE_PROGRAM_PATS ? process.env.OPEN_SOURCE_PROGRAM_PATS.trim()?.split(";") : []; // Array of GitHub Personal Access Tokens (PATs) (PAT;PAT;PAT...)

export const DATABASE_MAIN_HOST = process?.env?.DATABASE_MAIN_HOST!;
export const DATABASE_NEXT_HOST = process?.env?.DATABASE_NEXT_HOST!;
