...
const FETCH_PROJECTS_FROM_USER = new Elysia({ prefix: '' })
    ...
    .get('', async ({ fetchParams, params: { login_name }, query, set }) => {
        ...
        return response;
    }, ACCOUNT_LEVEL_HAVING_CHILDREN_OPTIONS("Request projects of the user. (`/projects?pageSize=1&continueAfter=abc`)"))

const ACCOUNT_LEVEL_CHILDREN = (login_type: "organization" | "user") => new Elysia({ prefix: '' })
    .use(RESOLVE_JWT)
    ...
    .group("/projects", (app) => app
        .use(login_type === "organization" ? FETCH_PROJECTS_FROM_ORGANIZATION : FETCH_PROJECTS_FROM_USER)
